...
- **只读优先：** 先报告空间占用、类别和大文件，再给出整理建议。
- **公共路径确认：** 每次会话先确认模型与数据集公共目录；默认分别为 `/publicdata/model`、`/publicdata/data`。
- **可靠去重：** 对模型和可复用数据集核对版本、文件清单、大小及 SHA-256，而非只看名称。
- **安全归档：** 公共目录缺少内容时，先复制并校验，再由用户确认是否删除个人副本。
- **保护共享目录：** 公共目录多人可写，Skill 不会自动删除、覆盖、重命名或移动其中内容。
- **谨慎清理：** checkpoint、微调权重和未知大文件只作为待审查项，绝不自动删除。
